...
for doc in docFiles:
    ...
    print(
        "\n the tf vector for document %d is [%s]"
        % ((docFiles.index(doc) + 1), tf_vector_string)
    )
...
print(
    "\nThe inverse documnet frequency vectir is ["
...
for i in range(len(docFiles)):
    for j in range(i + 1, len(docFiles)):
        result_nltk = nltk.cluster.util.cosine_distance(
            doc_term_matrix_tfidf_l2[i], doc_term_matrix_tfidf_l2[j]
        )
        print("\n Cosine Distance btw student %d and student %d:" % (i, j))
        print(result_nltk)
